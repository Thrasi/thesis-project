...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  deconv_weights = tf.Variable(
      tf.truncated_normal([5, 5, 512, 512],
                          stddev=0.1,
                          seed=SEED))
...
#    print("After first conv: "+str(relu.get_shape())
    ...
    # # 7x7
    deconv_shape=tf.constant([BATCH_SIZE,7,7,512])
    print(deconv_shape.get_shape())
    unpool = tf.nn.conv2d_transpose(conv,
                                    deconv_weights,
                                    output_shape=deconv_shape,
                                    strides=[1,1,1,1],
                                    padding='VALID')
    print("After trans: "+str(unpool.get_shape()))
